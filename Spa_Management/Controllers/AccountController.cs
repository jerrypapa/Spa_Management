...
namespace Spa_Management.Controllers
{
	...
	public class AccountController : Controller
	{
        ...
        public async Task<AppLoginResponse> BeneficiaryLogin(AppLogin model, string returnUrl = null)
        {
            try
            {
              ...
              //  ViewData["ReturnUrl"] = returnUrl;
                if (ModelState.IsValid)
                {
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        //if (object.ReferenceEquals(returnUrl, null))
                        //{
                        //    return RedirectToAction(nameof(merController.Index), "Mer");
                        //}
                    ...
                    {
                        LoginResponse.Code = "0001";
                        LoginResponse.Description = "Login Failed";
                        LoginResponse.InstitutionName = "";
                    }
                }
            }
		...
		public async Task<IActionResult> Login(LoginViewModel model, string returnUrl = null)
		{
			try
			{
				...
				if (ModelState.IsValid)
				{
					...
					{
						ModelState.AddModelError(string.Empty, "Wrong user Email or Password");
                      //  ModelState.AddModelError(string.Empty, "Invalid authenticator code.");
                        return View();
                        //	ModelState.(string.Empty, "Invalid login attempt.");
		...
		public async Task<IActionResult> Register(RegisterViewModel model, string returnUrl = null)
		{
			...
			if (ModelState.IsValid)
			{
				...
				if (result.Succeeded)
				{
					...
					var code = await _userManager.GenerateEmailConfirmationTokenAsync(user);
					var callbackUrl = Url.EmailConfirmationLink(user.Id, code, Request.Scheme);
					await _dbOps.LogAndSendMail(model.Email, "Email Confirmation","Dear "+user.UserName+ " ,Please Click on this link to verify your email address : > "+ callbackUrl);
		...
		public async Task<IActionResult> ForgotPassword(ForgotPasswordViewModel model)
		{
			if (ModelState.IsValid)
			{
				...
				await _dbOps.LogAndSendMail(model.Email, "Reset Password",
